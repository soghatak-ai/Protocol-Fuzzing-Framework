...
- rest_data_reuse: CSCwu90022 — REST offset reuse after ABOR, REST+PASV/PORT mode switching, multi-channel race with same offset, offset accumulation without transfer, concurrent RETR+STOR with shared offset. Targets Snort's file-position tracker when a REST offset is reused after data channel teardown. Triggers: use-after-free, state-corruption, oob-read
...
- partial_header_close: CSCwu90024 — truncated request line, partial Host header, truncated chunked TE header, partial Content-Length header, header flood then abrupt close. Sends incomplete HTTP headers then closes the connection. Targets header reassembly buffer cleanup. Triggers: use-after-free, oob-read, state-corruption
...
- resp_js_normalization_crash: CSCwu24006/CSCwu24015 — HTTP responses with malformed JavaScript bodies: malformed script tags with invalid Unicode escapes, deeply nested function closures (500+), invalid UTF-16 surrogate pairs, huge 60KB JS bodies, NUL bytes inside script content, mixed encoding. Targets Snort's JS normalization engine in http_inspect. Triggers: crash, oob-read, buffer-overflow
...
- session_rapid_reuse: CSCwu17809/CSCwu23995 — rapid SESSION_SETUP/LOGOFF cycles reusing SessionId, TREE_CONNECT on logged-off sessions, pipelined LOGOFF+SESSION_SETUP, SESSION_SETUP with stale SessionId, multi-channel session reuse. Targets SMB session state tracking for use-after-free in session teardown/reuse. Triggers: use-after-free, state-corruption, null-deref
- durable_reconnect_overflow: CSCwq01522 — SMB2 CREATE with oversized durable reconnect handle contexts: 60KB DH2Q/DH2C create contexts, undersized (1-byte) durable handle, multiple conflicting durable contexts, mismatched DH2Q+DH2C flags. Targets durable handle reconnect parsing for out-of-bounds read/write. Triggers: oob-read, heap-overflow, buffer-overflow
...
- connect_authority_overflow: CSCwu34640 — HTTP/2 CONNECT method with malformed :authority pseudo-header: 30KB oversized authority, null-byte-embedded authority, IPv6-bracket overflow, port-only (no host), missing :authority, duplicate authorities with conflicting values. Targets HTTP/2 CONNECT authority parsing for out-of-bounds read. Triggers: oob-read, heap-overflow, buffer-overflow
...
- smb_session_state_confusion: CSCwu17809/CSCwu23995 — DCE/RPC over SMB session confusion: BIND after SMB LOGOFF, pipelined SMB SESSION_SETUP+BIND+REQUEST, BIND on TreeId=0, REQUEST interleaved with SESSION_SETUP, BIND with stale SessionId. Targets DCE/RPC inspector when underlying SMB session is torn down. Triggers: use-after-free, state-corruption, null-deref
...
- kexinit_empty_algorithm: CSCwu90036 — KEXINIT with zero-length algorithm name-lists: all 10 lists empty, single empty kex_algorithms only, empty with valid cookie, oversized cookie + empty lists, valid banner then empty KEXINIT, truncated KEXINIT mid-namelist. Targets Snort SSH inspector's KEXINIT parser when algorithm strings are absent. Triggers: null-deref, oob-read, state-corruption
